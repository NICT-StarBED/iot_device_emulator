...
int main(int argc, char *argv[])
{
    ...
    if ((fp = fopen("/var/tmp/.sensor_id", "r")) == NULL) {
        num = 0;
    } else {
    ...
    if ((fp = fopen("/var/tmp/.sensor_id", "w")) != NULL) {
        fprintf(fp, "%d", num);
        fclose(fp);
    } else {
        fprintf(stderr, "sensor_id setting error: fopen\n");
        exit(EXIT_FAILURE);
    }

    if ((fp = fopen("../sensor_node/external/uuid.dat", "r")) == NULL) {
        fprintf(stderr, "sensor_id setting error: uuid file not found\n");
        exit(EXIT_FAILURE);
    }
    for (i = 0; i < num + 1; i++) {
        fscanf(fp, "%08x\n", &uuid);
    }
    printf("sensor number: %02d\n", num);
    printf("sensor node: %08x\n", uuid);
    fclose(fp);

    sprintf(buffer, "/var/tmp/.%08x.i2c", uuid);
    if((s = get_server_socket(buffer)) == -1) {
        exit(EXIT_FAILURE);
    }
